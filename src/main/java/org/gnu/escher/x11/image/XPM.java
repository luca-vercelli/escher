...
public class XPM extends ZPixmap { // TODO
	public XPM(Display display, String[] xpm, VisualInfo xVisual) {
		...
		Map<String, Color> mapping = new HashMap<>(2 * num_colors);
		...
		for (int i = 0; i < num_colors; i++) {
			...
			while (color.hasMoreTokens() && color.nextToken().equals("c")) { // can only parse c for now

				String color_value = color.nextToken();

				if (color_value.charAt(0) == '#') {
					int pixel = Integer.parseInt(color_value.substring(1, color_value.length()), 16);
		...
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width; x++) {
				String symbol = pixels.substring(x * chars_per_pixel, (x + 1) * chars_per_pixel);
