...
public class Display implements AutoCloseable {
	...
	/**
	 * Performs client connection to the XServer, initialising all the internal
	 * datastructures.
	 * 
	...
	/**
	 * This method returns a list of visual information that match the specified
	 * attributes given in the visual information template.
	 * 
	 * The list can be empty if no visual match the given template.
	 * 
	 * If template is null, it returns a list of all the visual information for the
	 * given screen, behaving like {@link #getVisualInfo()}.
	 * 
	 *
	 * @param template
	 * @return
	 */
	public List<VisualInfo> getVisualInfo(VisualInfo template, int visualInfoMask) {
			...
			for (Depth depth : screen.getDepths()) {
				...
				for (VisualInfo visual : depth.getVisuals()) {
					...
					if (((visualInfoMask & VisualInfoMask.VisualClassMask) != 0)
							&& (!template.getVisualClass().equals(visual.getVisualClass())))
						continue;

					if (((visualInfoMask & VisualInfoMask.VisualRedMaskMask) != 0)
							&& (template.getRedMask() != visual.getRedMask()))
						continue;

					if (((visualInfoMask & VisualInfoMask.VisualGreenMaskMask) != 0)
							&& (template.getGreenMask() != visual.getGreenMask()))
						continue;

					if (((visualInfoMask & VisualInfoMask.VisualBlueMaskMask) != 0)
					...
					if (((visualInfoMask & VisualInfoMask.VisualColormapSizeMask) != 0)
							&& (template.getColormapEntries() != visual.getColormapEntries()))
						continue;

					if (((visualInfoMask & VisualInfoMask.VisualBitsPerRGBMask) != 0)
							&& (template.getBitsPerRGBValue() != visual.getBitsPerRGBValue()))
						continue;
	...
	/**
	 * Returns a list of all the Visuals supported by this Display that match this
	 * template.
	 * 
	...
	/**
	 * Return a single Visual associated to the given id. This is an optimized
	 * version of {@link #getVisualInfo(VisualInfo, int)}, when the id is known in
	 * advance. This method may return {@code null}, as opposed to the other
	 * {@code getVisualInfo()}, which always return at least a List with no entries.
	 */
	...
	/**
	 * Determines if the named extension is present. If so, the major opcode for the
	 * extension is returned, if it has one. Otherwise zero is returned. Any minor
	 * opcode or the request formats are specific to the extension. If the extension
	 * involves additional event types, the base event type code is returned.
	 * Otherwise zero is returned. The format of the events is specific to the
	 * extension. If the extension involves additional error codes, the base error
	 * code is returned. The format of additional data in the errors is specific to
	...
	// opcode 104 - bell
	/**
	 * Rings the bell on the keyboard at a volume relative to the base volume of the
	 * keyboard, if possible. Percent can range from -100 to +100 inclusive (or a
	 * Value error results). The volume at which the bell is rung when percent is
	 ...
	 */
	public void setScreenSaver(int timeout, int interval, ScreenSaverBlanking preferBlanking,
	...
	/**
	 * Returns the hosts currently on the access control list and whether use of the
	 ...
	 * client's resources inside the X server. Xlib hands these out sequentially as
	 * needed. When it overruns the end of the range, an IDChoice protocol error
	 * results. Long running clients, or clients that use resource IDs at a rapid
	 * rate, may encounter this circumstance. When it happens, there are usually
	 * many resource IDs available, but Xlib doesn't know about them. One approach
	 * to solving this problem would be to have Xlib notice when a resource is freed
	 * and recycle its ID for future use. This strategy runs into difficulties
	 * because sometimes freeing one resource causes others to be freed (for
	 * example, when a window is destroyed, so are its children). To do a complete
	 * job, Xlib would have to maintain a large amount of state that currently
	 * resides only in the server (the entire window tree in the above example).
	 * Even if a less comprehensive strategy was adopted, such as recycling only
	 * those IDs that Xlib can identify without maintaining additional state, the
	 * additional bookkeeping at resource creation and destruction time would likely
	 * introduce unacceptable overhead. To avoid the problems listed above, the
	 * server's complete knowledge of all resource IDs in use by a client is
	 * leveraged. This extension provides two ways for Xlib to query the server for
	 * available resource IDs. Xlib can use these extension requests behind the
	...
	public int allocateID(Object object) {
		 ...
		 * resource ID sequentially to the end without recycling ID (just as xlib does).
		 * Sample values: resource base: 0x04000000 or 00000100000000000000000000000000b
		...
		if (xcmiscResourceCount == 0) {
			...
			org.gnu.escher.x11.extension.XCMisc.XIDRange rr = xcmisc.xidRange();
	...
	/**
	 * From Big Requests extension specification: It is desirable for core Xlib, and
	 * other extensions, to use this extension internally when necessary. It is also
	 * desirable to make the use of this extension as transparent as possible to the
	 * X client. For example, if enabling of the extension were delayed until the
	 * first time it was needed, an application that used XNextRequest to determine
	 * the sequence number of a request would no longer get the correct sequence
	 * number. As such, XOpenDisplay will determine if the extension is supported by
	 * the server and, if it is, enable extended-length encodings. The core Xlib
	...
	/**
	 * Reads the server information after connection setup. The information is read
	 * from the connection's ResponseInputStream.
	 */
	private void initServerInfo(ResponseInputStream i) {
		...
		case 0:
			...
			debugMessage.append("Try to set DISPLAY variable or to give " + "proper\n");
			debugMessage.append("permissions (eg. edit .Xauthority or " + "run \"xhost +\")\n");
			debugMessage.append("NOTE: leaving \"xhost +\" and allowing " + "remote tcp\n");
			debugMessage.append("connections to the XServer can be a " + "potential\n");
		...
		if (connectionFailed) {
			...
			debugMessage.append("XServer returned " + failedLength + " error ");
			...
			for (int n = 0; n < failedLength; n++) {
				...
				debugMessage.append("XServer returned error code: " + reason + "\n");
			}

			debugMessage.append("XServer are allowed to use non " + "standard errors codes\n");
			debugMessage.append("Please, consult the manual of your " + "XServer\n" + "to map the error codes to human "
					+ "readable values.\n");
		...
		if (defaultScreenNumber < 0 || defaultScreenNumber >= screen_count)
			throw new RuntimeException(
					"Invalid screen number (screen-count " + screen_count + "): " + defaultScreenNumber);
	...
	public String toString() {
		...
		return "#Display" + "\n  default-screen-number: " + defaultScreenNumber + "\n  vendor: " + vendor
				+ "\n  release-number: " + releaseNumber + "\n  maximum-request-length: " + maximumRequestLength;
	}
	...
	public void check_error() {
		...
		try {
			...
			/*
			 * When an X error occurs, Java throws an `gnu.x11.Error' exception, the normal
			 * execution order is disrupted; the reply of `input_focus()' resides in network
			 * buffer while nobody wants it. In case someone (`gnu.x11.test.Shape') catches
			 * the error and continues to work, we should discard the input focus reply (by
			 * clearing the socket input stream). TODO Should I be careful not to clear
			 * other packets after the reply of input focus? Some event may come after that?
			 */
			try {
