...
public class DisplayName {
	...
	/**
	 * Connects to socket and creates new Display. If hostName is null or
	 * "localhost" hostName is resolved before connecting. If socketPath is set a
	 * unix socket is used otherwise a tcp socket is used.
	 * 
	 * @return
	 */
	...
	public DisplayName withHostName(String hostName) {
		return this.hostName.equals(hostName) ? this
				: new DisplayName(hostName, displayNumber, screenNumber, socketFile);
	}

	public DisplayName withDisplayNumber(int displayNumber) {
		return this.displayNumber == displayNumber ? this
				: new DisplayName(hostName, displayNumber, screenNumber, socketFile);
	}

	public DisplayName withScreenNumber(int screenNumber) {
		return this.screenNumber == screenNumber ? this
				: new DisplayName(hostName, displayNumber, screenNumber, socketFile);
	}

	public DisplayName withSocketFile(File socketFile) {
		return this.socketFile.equals(socketFile) ? this
				: new DisplayName(hostName, displayNumber, screenNumber, socketFile);
	}
}
