...
public class MiscKeySym {
	 ...
	 *
	 * (defun mixed-case-to-lisp-case (string) "Convert SomeMixedCaseString to
	 * a_more_readable_lisp_case_string. E.g. XK_Armenian_ligature_ew _>
	 * xk_armenian_ligature_ew." (downcase (char-list-to-string (loop with previous
	 * = ? for current in (string-to-char-list string) if (and (lowercasep previous)
	 * (uppercasep current)) collect ?_ and collect current else collect current do
