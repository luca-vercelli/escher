...
import org.gnu.escher.x11.Screen;
...
public class Window extends Drawable implements GLXDrawable {
	 ...
	 * 
	 * <p>
	 * All predefined resources are not "properly" initialized, in the sense that
	 * member variable <code>display</code> is <code>null</code>. That is, they are
	 * not connected to any X server, and cannot be used for server interaction
	 * (because <code>display == null</code>, resulting in
	 ...
	 * However, predefined resources are not designed in the first place for server
	 * interaction. For example, you cannot draw a line on
	 * <code>Window.POINTER_WINDOW</code>. Second, it is a better classification to
	 * put predefined resources in their respective classes. Third, it is cheaper to
	 * have predefined resources as <code>static final</code> objects.
	 * 
	 * <p>
	 ...
	 * (this, 0)</code> will not return static variable {@link #NONE}. Instead it
	 * creates a new <code>Window</code> object which sequential intern calls will
	 * return. As a side effect, <code>Window</code> equality should not be tested
	...
	public class Property implements InputStreamObject {
		...
		/**
		 * Returns the value at index <code>i</code>. This interprets the underlying
		 * byte data according to the format of this property.
		 * 
		 * @param i the index
		 * 
		...
		public int value(int i) {
			...
			case 16:
				...
				break;
			case 32:
				v = ((0xff & value[i * 4]) << 24) | ((0xff & value[i * 4 + 1]) << 16) | ((0xff & value[i * 4 + 2] << 8))
	...
	public static class WMHints extends Data {
		 ...
		 *         {@link #ICON_PIXMAP_HINT_MASK}, {@link #ICON_WINDOW_HINT_MASK},
		 *         {@link #ICON_POSITION_HINT_MASK}, {@link #ICON_MASK_HINT_MASK},
	...
	public Window(Window parent, int x, int y, int width, int height, int borderWidth, WindowAttributes attr) {
	 ...
	 * @param depth
	 * 
	 * @param klass    valid: {@link WinClass}
	 * 
	 * @param visualID
	 * 
	 * @see <a href="XCreateWindow.html">XCreateWindow</a>
	 */
	public void create(int borderWidth, int depth, WinClass klass, int visualID, WindowAttributes attr) {
	...
	/**
	 * Extra parameters (offset and data_format) are used to support Data class as
	 * parameter for writing. See setWMNormalHints ().
	 * 
	 * @param mode
	 * 
	 * @param format:      valid: <code>8</code>, <code>16</code>, <code>32</code>
	 * 
	 * @param data_format: valid: <code>8</code>, <code>16</code>, <code>32</code>
	 * 
	 * @see <a href="XChangeProperty.html">XChangeProperty</a>
	 */
	public void changeProperty(PropertyMode mode, Atom property, Atom type, int format, Object data, int offset,
			int data_format) {
		...
		case 32:
			...
			break;
		default:
			throw new IllegalArgumentException("Illegal format argument: " + format);
		}
		int len = 0;
	 ...
	 */
	public Property getProperty(boolean delete, Atom property, Atom type, int offset, int length) {
	 ...
	 */
	public void convertSelection(Atom selection, Atom target, Atom property, int time) {
	 ...
	 * @param keyboardMode
	 * 
	 * @param confineTo    possible: {@link #NONE}
	 * @param cursor       possible: {@link Cursor#NONE}
	 * @param time         possible: {@link Display#CURRENT_TIME}
	 * 
	 * @see <a href="XGrabPointer.html">XGrabPointer</a>
	 */
	public GrabStatus grabPointer(boolean owner_events, int eventMask, GrabMode pointerMode, GrabMode keyboardMode,
	 ...
	 * @param pointerMode  valid: {@link GrabMode#SYNCHRONOUS},
	 *                     {@link GrabMode#ASYNCHRONOUS}
	 * 
	 * @param keyboardMode valid: {@link GrabMode#SYNCHRONOUS},
	 *                     {@link GrabMode#ASYNCHRONOUS}
	 * 
	 * @param confineTo    possible: {@link #NONE}
	 * @param cursor       possible: {@link Cursor#NONE}
	 ...
	 */
	public void grabButton(int button, int modifiers, boolean ownerEvents, int eventMask, GrabMode pointerMode,
			GrabMode keyboardMode, Window confineTo, Cursor cursor) {
	 ...
	 * @param time         possible: {@link Display#CURRENT_TIME}
	 * 
	 * @return valid: {@link GrabStatus#SUCCESS},
	 *         {@link GrabStatus#ALREADY_GRABBED}, {@link GrabStatus#FROZEN},
	 *         {@link GrabStatus#INVALID_TIME}, {@link GrabStatus#NOT_VIEWABLE}
	 * 
	 * @see <a href="XGrabKeyboard.html">XGrabKeyboard</a>
	 */
	public GrabStatus grabKeyboard(boolean ownerEvents, GrabMode pointerMode, GrabMode keyboardMode, int time) {
	 ...
	 */
	public void grabKey(int keysym, int modifiers, boolean ownerEvents, GrabMode pointerMode, GrabMode keyboardMode) {
	...
	/**
	 * @param keysym    possible: {@link #ANY_KEY}
	 * @param modifiers possible: {@link #ANY_MODIFIER}
	 ...
	 */
	public void warpPointer(Window src, int srcX, int srcY, int srcWidth, int srcHeight, int destX, int destY) {
	 ...
	 */
	public void clearArea(int x, int y, int width, int height, boolean exposures) {
	...
	public void changeProperty(Atom property, Atom type, int data) {
		...
		changeProperty(PropertyMode.REPLACE, property, type, 32, new int[] { data }, 0, 32);
	}
	 ...
	 */
	public void grabButtonIgnoreLocks(int button, int modifiers, boolean ownerEvents, int eventMask,
			GrabMode pointerMode, GrabMode keyboardMode, Window confineTo, Cursor cursor) {
		...
		for (int i = 0; i < Input.LOCK_COMBINATIONS.length; i++)
			grabButton(button, modifiers | Input.LOCK_COMBINATIONS[i], ownerEvents, eventMask, pointerMode,
	 ...
	 *      Window, Cursor)
	 */
	public void grabKeyIgnoreLocks(int keysym, int modifiers, boolean ownerEvents, GrabMode pointerMode,
			GrabMode keyboard_mode) {

		for (int i = 0; i < Input.LOCK_COMBINATIONS.length; i++)
			grabKey(keysym, modifiers | Input.LOCK_COMBINATIONS[i], ownerEvents, pointerMode, keyboard_mode);
	}
	...
	public void moveResize(Rectangle rectangle) {
		...
		moveResize(rectangle.getX(), rectangle.getY(), rectangle.getWidth(), rectangle.getHeight());
	}
	...
	public void setWMClassHint(String res_name, String res_class) {
		...
		changeProperty(PropertyMode.REPLACE, Atom.WM_CLASS, Atom.STRING, 8, wm_class.getBytes(), 0, 8);
	}
	...
	public void set_wm_hints(WMHints wm_hints) {
		...
		changeProperty(PropertyMode.REPLACE, Atom.WM_HINTS, Atom.WM_HINTS, 8, wm_hints.getData(), 32, 8);
	}
	...
	public void setWMNormalHints(WMSizeHints sizeHints) {
		...
		changeProperty(PropertyMode.REPLACE, Atom.WM_NORMAL_HINTS, Atom.WM_SIZE_HINTS, 32, sizeHints.getData(), 32, 8);
	}
	...
	public void setWMName(String wmName) {
		...
		changeProperty(PropertyMode.REPLACE, Atom.WM_NAME, Atom.STRING, 8, wmName.getBytes(), 0, 8); // support other
																										// types?
	}
	...
	public void setWMState(WMInitialState state, Window icon) {
		...
		int[] data = { state.getCode(), icon.id };
	...
	public String toString() {
		...
		return "#Window " + id + " " + (new Rectangle(x, y, width, height)).spec();
	}
