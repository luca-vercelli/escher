...
public class RequestOutputStream extends FilterOutputStream {
	...
	/**
	 * TimerTask for forcing flush of this RequestOutputStream after at most
	 * FLUSH_TIMER_DELAY milliseconds.
	 */
	 ...
	 */
	public enum Buffer {
		DEFAULT_BUFFER_SIZE(512), MAX_BUFFER_SIZE(8192), FLUSH_THRESHOLD(64);
	...
	/**
	 * Defines the GLX opcode as returned by the server, in case we are using the
	 * GLX extension.
	 */
	...
	/**
	 * The request buffer. It always holds the current request. This can be accessed
	 * directly for modifications, like when the current request can be aggregated.
	 */
	private byte[] buffer;

	/**
	 * The current write index in the buffer. This always points to the next free
	 * location inside the buffer.
	 */
	public int index;
	 ...
	 * escher.send_mode that can have the values SYNCRONOUS, ASYNCRONOUS and
	 * ROUND_TRIP. If no such property is specified, the send mode is set to
	 * ASYNCRONOUS, which is the recommended mode for applications. The other two
	 * modes are useful for debugging.
	 *
	...
	/**
	 * Performs the same operation as {@link #send()} but without possibly doing a
	 * round-trip check.
	 */
	void sendImpl() {
		...
		if (index > requestIndex) {
			...
			if (pad != 0)
				skip(pad);
			requestIndex = index;
			if (index > (buffer.length - Buffer.FLUSH_THRESHOLD.getSize()) || sendMode == SendMode.SYNCHRONOUS
	...
	/**
	 * This forces a reply from the X server by sending an input focus request. This
	 * is used for the {@link SendMode#ROUND_TRIP} mode, which is very useful for
	 * debugging because the X errors can be traced to their corresponding calls.
	 */
	...
	public int getInt32(int index) {
		...
		int int32 = (buffer[req_index] << 24) | (buffer[req_index + 1] << 16) | (buffer[req_index + 2] << 8)
				| buffer[req_index + 3];
		return int32;
	}
	...
	public void increaseLength(int i) {
		int l = (((int) (buffer[requestIndex + 2] & 0xff)) << 8) + (buffer[requestIndex + 3] & 0xff);
		l += i;
	 ...
	 * <code>null</code>. Note that most requests are not implemented using request
	 * objects.
	 *
