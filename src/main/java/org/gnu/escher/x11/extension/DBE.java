 ...
 * <a href= "http://escher.sourceforge.net/etc/specification/dbe-library.ps.gz"
 * >here</a> (<a href=
 * "http://escher.sourceforge.net/etc/specification/dbe-protocol.ps.gz"
 * >protocol</a>).
 * 
 * <p>
 * Check <a href="../../../../etc/faq/dbe">this</a> for an insightful discusson
 * on the merit of DBE.
 */
public class DBE extends Extension implements ErrorFactory {
	...
	public enum SwapAction {
		UNDEFINED(0), BACKGROUND(1), UNTOUCHED(2), COPIED(3);
	...
	public DBE(org.gnu.escher.x11.core.Display display) throws ExtensionNotFoundException {
	...
	/**
	 * Get information about which visuals support double buffering. There seems to
	 * be <a href= "../../../../etc/dbe-bug">bug</a> in all servers based on X
	 * Consortium sample implementation up to R6.5 (including XFree86 4.0.1 or
	 * earlier): the reply length of the reply is incorrect , causing a "read timed
	 * out" error.
	 *
	...
	public static final String ERROR_STRING = "BAD_DBE_BUFFER: parameter not a DBE back buffer";

	public X11ServiceException build(org.gnu.escher.x11.core.Display display, int code, int seqNumber, int bad,
			int minorOpcode, int majorOpcode) {
		ErrorCode errorCd = ErrorCode.getError(code);
		return new X11ServiceException(display, ERROR_STRING, errorCd, seqNumber, bad, minorOpcode, majorOpcode);
	}
