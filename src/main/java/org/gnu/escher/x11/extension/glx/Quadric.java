 ...
 *
 * <p>
 * Modified from <code>src-glu/quadric.c</code> in
 * <a href= "http://mesa3d.org/">Mesa 3D</a> by Brian Paul.
 */
public class Quadric {
	 ...
	 */
	public synchronized void cylinder(double base_radius, double top_radius, double height, int slices, int stacks) {
	 ...
	 */
	public synchronized void disk(double inner_radius, double outer_radius, int slices, int loops) {
	 ...
	 */
	public synchronized void partial_disk(double inner_radius, double outer_radius, int slices, int loops, double start,
			double sweep) {
	...
	public synchronized void sphere(double radius, int slices, int stacks) {
		 ...
		 * implementation, because of different vertex ordering. There is visible
		 * discrepency particularly for the specular reflection with GL.FLAT shading
		 * model in `accanti.c' and `accpersp.c' in redbook. Brain Paul claims both are
		 * acceptable.
		 */
	...
	private void sphere_fill() {
		...
		if (!texture)
			sphere_fill_draw_positive_z();
		sphere_fill_draw_stack();
		if (!texture)
			sphere_fill_draw_negative_z();
	}
