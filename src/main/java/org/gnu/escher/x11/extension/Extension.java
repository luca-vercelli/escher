...
abstract public class Extension {
	...
	protected Extension(Display display, String name, String[] minorOpcodeStrings) throws ExtensionNotFoundException {

		this(display, name, minorOpcodeStrings, 0, 0);
	}

	protected Extension(Display display, String name, String[] minorOpcodeStrings, int errorCount, int eventCount)
		...
		display.extensionMinorOpcodeStrings[majorOpcode - 128] = minorOpcodeStrings;

		// register error factory
		for (int i = 0; i < errorCount; i++)
			display.extensionErrorFactories[firstError - 128 + i] = (ErrorFactory) this;

		// register event factory
		for (int i = 0; i < eventCount; i++)
			display.extensionEventFactories[firstEvent - 64 + i] = (EventFactory) this;
	}

	/**
	 * Additional information such as client version and server version to display
	...
	public String toString() {
		return "#Extension \"" + name + "\" " + "\n  major-opcode: " + majorOpcode + "\n  first-event: " + firstEvent
				+ "\n  first-error: " + firstError + moreString();
	}
