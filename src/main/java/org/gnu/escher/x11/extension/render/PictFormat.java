 ...
 * directly. Instead you can query the available formats by using ... and use
 * one of those. In order to make format selection easier, you can create a
 * {@link Template} and use
...
public class PictFormat {
	...
	public static class Direct {
		...
		/**
		 * Checks if the direct parameters in the template matches our parameters.
		 *
		...
		boolean match(Template temp) {
			return temp.red_shift == red_shift && temp.red_mask == red_mask && temp.green_shift == green_shift
					&& temp.green_mask == green_mask && temp.blue_shift == blue_shift && temp.blue_mask == blue_mask
		...
		public String toString() {
			return "Direct[red_shift=" + red_shift + ", red_mask=" + red_mask + ", green_shift=" + green_shift
					+ ", green_mask=" + green_mask + ", blue_shift=" + blue_shift + ", blue_mask=" + blue_mask
	...
	public static class Template {
		...
		/**
		 * Creates a new template for format selection. The shift and mask arguments
		 * only need to be specified for direct formats (otherwise, leave them 0). Of
		 * the shifts and masks arguments, only the least 16 significant bits are used.
		 *
		...
		public void set_direct(int red_shift, int red_mask, int green_shift, int green_mask, int blue_shift,
	 ...
	 */
	public String toString() {
		return "PictFormat[type=" + type + ", depth=" + depth + ", direct=" + direct + "]";
	}
