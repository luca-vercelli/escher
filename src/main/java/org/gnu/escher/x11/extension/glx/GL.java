 ...
 * <a href= "http://escher.sourceforge.net/etc/specification/gl-1.2.1.ps.gz"
 * >here</a>
 * (<a href= "http://escher.sourceforge.net/etc/specification/gl-design.ps.gz"
 * >design</a>).
 * 
 * <p>
...
public class GL extends org.gnu.escher.x11.resource.Resource implements GLConstant {
	...
	private class LargeRenderRequest {
		...
		/**
		 * Indicates if we are currently writing the large parameter. In this case we
		 * need to check for splitting up the value.
		 */
		...
		/**
		 * Prepares the LargeRenderRequest for sending a new request. This starts the
		 * request and writes the render opcode and render request length.
		 * 
		 * @param o                  the output stream
		 * @param l                  the length of the request
		 * @param large_param_length the length of the large parameters in bytes
		 */
		void begin(RequestOutputStream o, int opcode, int small_params_length, int large_param_length) {
			...
			if (render_large) {
				...
				// parameter request.
				request_total = (large_param_length + largeRequestLength - 1) / largeRequestLength + 1;
				...
				out.beginRequest(getGLX().getMajorOpcode(), 1, 2 + render_command_length / 4);
				out.writeInt32(getTag());
	...
	/**
	 * Starts a render request. First this makes sure that the current request is a
	 * render request, and that the buffer has enough room for the new render
	 * command. Then it writes the opcode and length to the request and returns the
	 * stream for the caller to complete the render command.
	 * 
	 * If the current request is a render request, but the buffer hasn't got enough
	 * room for the new command, or the current request is no GLX render command,
	 * the buffer is flushed and a new request is started.
	 * 
	 ...
	 */
	private GLRenderRequest beginRenderRequest(RequestOutputStream o, GLXRenderingCommand command) {
	 ...
	 */
	@Deprecated
	private GLRenderRequest begin_render_request(RequestOutputStream o, int opcode, int length) {
	...
	private void beginRenderRequest(RequestOutputStream o, GLRenderRequest rr) {
		...
		// in the buffer.
		o.beginRequest(glx.getMajorOpcode(), 1, GLRenderRequest.DEFAULT_BUFFER_SIZE);
		rr.reset(tag);
	 ...
	 */
	public GL(GLX glx, int visual_id, int screen_no, GL share_list, boolean direct) {
	 ...
	 * GLX pixmap. As a result of these two actions, subsequent GL rendering calls
	 * use rendering context ctx to modify GLX drawable drawable. Because
	 * glXMakeCurrent always replaces the current rendering context with ctx, there
	 * can be only one current context per thread.
	 * 
	 * glx opcode 5 - make current
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/glx/xmakecurrent.html">glXMakeCurrent</a>
	 */
	 ...
	 * execution. Display lists are created with glNewList. All subsequent commands
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/newlist.html">glNewList</a>
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glNewList.xml">glEndList</a>
	 */
	@Deprecated
	 ...
	 * contiguous empty display lists, named n, n+1, ..., n+range -1, are created.
	 * 
	 * GLX opcode 104.
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/genlists.html">glGenLists</a>
	 */
	 ...
	 * glDrawPixels and glReadPixels as well as the unpacking of polygon stipple
	 * patterns (see glPolygonStipple), bitmaps (see glBitmap), and texture patterns
	 * (see glTexImage1D, glTexImage2D, glTexSubImage1D, and glTexSubImage2D).
	 * 
	 * GLX Opcode 110
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/pixelstore.html">glPixelStorei</a>
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glGet.xml">glGetIntegerv</a>
	 */
	@Deprecated
	 ...
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glGetLight.xml">glGetLightfv</a>
	 */
	@Deprecated
	 ...
	 * @param paramsOffset
	 */
	public void getLightfv(int light, int pname, float[] params, int paramsOffset) {

		this.getFloatVector(GLXCommand.GetLightfv, light, pname, params, paramsOffset);
	}
	...
	/**
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glGetPixelMap.xml.html">
	 *      glGetPixelMapuiv</a>
	 */
	@Deprecated
	...
	/**
	 * Returns in <code>params</code> the contents of the pixel map specified in map
	 * 
	 * GLX opcode 126.
	 * 
	 * @see <a href=
	...
	/**
	 * Returns pixel data from the frame buffer, starting with the pixel whose lower
	 * left corner is at location (x, y), into client memory starting at location
	 * pixels.
	 * 
	 * @see <a href=
			...
			synchronized (in) {
				...
				if (pixels instanceof ByteBuffer) {
					...
					throw new UnsupportedOperationException(
							"Not Yet " + "Implemented for Buffer types != ByteBuffer " + "or IntBuffer");
				}
			}
		}
	}
	...
	/**
	 * glGetString returns a string describing some aspect of the current GL
	 * connection.
	 * 
	 * GLX opcode 129.
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/getstring.html">glGetString</a>
	 */
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/istexture.html">glXIsTexture</a>
	 */
	...
	public void call_list(int list) {
		...
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.CallList);
			rr.writeInt32(list);
		}
	}
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/calllist.html">glCallList</a>
	 */
	...
	/**
	 * glBegin and glEnd delimit the vertices that define a primitive or a group of
	 * like primitives.
	 * 
	 * GLX render opcode 4.
	 * 
	...
	public void begin(int mode) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = this.beginRenderRequest(o, GLXRenderingCommand.Begin);
			rr.writeInt32(mode);
		}
	}
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/color.html">glColor3f</a>
	 */
	 ...
	 */
	public void color4ub(boolean red, boolean green, boolean blue, boolean alpha) {
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glNormal.xml">glNormal3f</a>
	 */
	public void normal3f(float x, float y, float z) {

		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.Normal3fv);
			rr.writeFloat(x);
			rr.writeFloat(y);
			rr.writeFloat(z);
		}
	...
	public void normal3i(int x, int y, int z) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.Normal3iv);
			rr.writeInt32(x);
			rr.writeInt32(y);
			rr.writeInt32(z);
		}
	 ...
	 * and polygon vertices. The current color, normal, and texture coordinates are
	 * associated with the vertex when glVertex is called.
	 * 
	 * GLX Render opcode 70.
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/vertex.html">glVertex3f</a>
	 */
	...
	public void materialfv(int face, int pname, float[] params, int offset) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.Materialfv, 4 * n);
			rr.writeInt32(face);
			rr.writeInt32(pname);
			for (int i = 0; i < n; i++)
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/shademodel.html">glShadeModel</a>
	...
	public void shade_model(int mode) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.ShadeModel);
			rr.writeInt32(mode);
		}
	}
	 ...
	 */
	public void tex_image_1d(int target, int level, int internal_format, int width, int border, int format, int type,
			byte[] pixels) {
	 ...
	 */
	public void tex_image_2d(int target, int level, int internal_format, int width, int height, int border, int format,
	...
	public void clear(int mask) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.Clear);
			rr.writeInt32(mask);
		}
	}
	...
	/**
	 * glClearColor specifies the red, green, blue, and alpha values used by glClear
	 * to clear the color buffers.
	 * 
	 * GLX Render opcode 130.
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/clearcolor.html">glClearColor</a>
	 */
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glClearDepth.xml">glClearDepth</a>
	...
	public void clear_depth(double depth) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.ClearDepth);
	 ...
	 */
	public void color_mask(boolean red, boolean green, boolean blue, boolean alpha) {
	 ...
	 * flag is GL_FALSE, depth buffer writing is disabled. Otherwise, it is enabled.
	 * Initially, depth buffer writing is enabled.
	 * 
	 * GLX Render opcode 135.
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/depthmask.html">glDepthMask</a>
	...
	public void depth_mask(boolean flag) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.DepthMask);
			rr.writeBool(flag);
			rr.writePad(3);
		}
	}
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/enable.html">glEnable</a>
	 */
	public void enable(int capability) {

		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.Enable);
			rr.writeInt32(capability);
		}
	}
	 ...
	 */
	public void map1d(int target, double u1, double u2, int stride, int order, double[] points) {
	 ...
	 */
	public void map1f(int target, float u1, float u2, int stride, int order, float[] points) {
	 ...
	 */
	public void map2d(int target, double u1, double u2, int ustride, int uorder, double v1, double v2, int vstride,
	 ...
	 */
	public void map2f(int target, float u1, float u2, int ustride, int uorder, float v1, float v2, int vstride,
	 ...
	 */
	public void map_grid2d(int un, double u1, double u2, int vn, double v1, double v2) {
	 ...
	 */
	public void map_grid2f(int un, float u1, float u2, int vn, float v1, float v2) {
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/readbuffer.html">glReadBuffer</a>
	...
	public void read_buffer(int mode) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.ReadBuffer);
			rr.writeInt32(mode);
		}
	}
	 ...
	 */
	public void draw_pixels(int width, int height, int format, int type, byte[] pixels) {
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/frustum.html">glFrustum</a>
	 */
	public void frustum(double left, double right, double bottom, double top, double near, double far) {

		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.Frustum);
			rr.writeDouble(left);
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/loadidentity.html">glLoadIdentity</a>
	...
	public void matrix_mode(int mode) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.MatrixMode);
			rr.writeInt32(mode);
		}
	}
	...
	public void multMatrixd(DoubleBuffer buffer) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = beginRenderRequest(o, GLXRenderingCommand.MultMatrixd);
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glOrtho.xml">glPopMatrix</a>
	 */
	 ...
	 * GL_COLOR, GL_PROJECTION, and GL_TEXTURE, the depth is at least 2. The current
	 ...
	 * current matrix. That is, after a glPushMatrix call, the matrix on top of the
	 * stack is identical to the one below it.
	 * 
	 * glPopMatrix pops the current matrix stack, replacing the current matrix with
	 * the one below it on the stack.
	 * 
	 * GLX Render opcode 183.
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/sdk/docs/man/xhtml/glPopMatrix.xml">glPopMatrix</a>
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/rotate.html">glRotatef</a>
	 */
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/translate.html">glTranslatef</a>
	 */
	 ...
	 * 
	 * @see <a href=
	 *      "http://www.opengl.org/documentation/specs/man_pages/hardcopy/GL/html/gl/viewport.html">glViewport</a>
	 */
	 ...
	 */
	public void copy_color_sub_table(int target, int start, int x, int y, int width) {
	 ...
	 */
	public void multi_tex_coord4d_arb(int target, double s, double t, double r, double q) {
	 ...
	 */
	public void multi_tex_coord4f_arb(int target, float s, float t, float r, float q) {
	 ...
	 */
	public void copy_color_table(int target, int internal_format, int x, int y, int width) {
	 ...
	 */
	public void tex_subimage_2d(int target, int level, int xoffset, int yoffset, int width, int height, int format,
			int type, byte[] pixels) {
	 ...
	 */
	public void copy_convolution_filter1d(int target, int internal_format, int x, int y, int width) {
	 ...
	 * @see <a href="glCopyConvolutionFilter1d.html"> glCopyConvolutionFilter1d</a>
	 */
	public void copy_convolution_filter2d(int target, int internal_format, int x, int y, int width, int height) {
	 ...
	 */
	public void histogram(int target, int width, int internal_format, boolean sink) {
	...
	public void prioritize_textures(int[] textures, float[] priorities) {
		...
		RequestOutputStream o = display.getResponseOutputStream();
		synchronized (o) {
			GLRenderRequest rr = begin_render_request(o, 4118, 8 + textures.length * 2 * 8);
	 ...
	 */
	public void copy_texture_image_1d(int target, int level, int internal_format, int x, int y, int width, int border) {
	 ...
	 */
	public void copy_texture_image_2d(int target, int level, int internal_format, int x, int y, int width, int height,
			int border) {
	 ...
	 */
	public void copy_texture_sub_image_1d(int target, int level, int xoffset, int x, int y, int width) {
	 ...
	 */
	public void copy_texture_sub_image_2d(int target, int level, int xoffset, int yoffset, int x, int y, int width,
			int height) {
	 ...
	 */
	public void copy_texture_sub_image3d(int target, int level, int xoffset, int yoffset, int zoffset, int x, int y,
	...
	public boolean support(int major, int minor) {
		...
		String[] versions = org.gnu.escher.utils.Misc.tokenize(version_number, ".");
	...
	/**
	 * A generic function for a common request pattern in GLX. This sends a request
	 * that takes one int-like parameters (a 4 bytes) and returns a FLOAT32 array.
	 *
	 * @param opcode the opcode
	...
	/**
	 * A generic function for a common request pattern in GLX. This sends a request
	 * that takes two int-like parameters (a 4 bytes) and returns a FLOAT32 array.
	 *
	 * @param opcode the opcode
	 * @param par1   the first parameter
	...
	private void getFloatVector(GLXCommand command, int param1, int param2, float[] ret, int offset) {
	...
	/**
	 * A generic function for a common request pattern in GLX. This sends a request
	 * that takes two int-like parameters (a 4 bytes) and returns a FLOAT64 array.
	 *
	 * @param opcode the opcode
	 * @param par1   the first parameter
	 *
	...
	/**
	 * A generic function for a common request pattern in GLX. This sends a request
	 * that takes one int-like parameters (a 4 bytes) and returns a INT32 array.
	 *
	 * @param opcode the opcode
	 * @param par1   the first parameter
	...
	private void getIntVector(GLXCommand command, int param1, int[] ret, int offset) {
	...
	/**
	 * A generic function for a common request pattern in GLX. This sends a request
	 * that takes two int-like parameters (a 4 bytes) and returns a INT32 array.
	 *
	 * @param opcode the opcode
	 * @param par1   the first parameter
	 * @param par2   the second parameter
	...
	private void render3f(GLXRenderingCommand command, float p1, float p2, float p3) {
	...
	private void render_4d(int opcode, double p1, double p2, double p3, double p4) {
	...
	private void render4f(GLXRenderingCommand command, float p1, float p2, float p3, float p4) {
	...
	private void render4i(GLXRenderingCommand command, int p1, int p2, int p3, int p4) {
