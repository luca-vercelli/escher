 ...
 * res_name and res_class given. I use this all the time to send common long
 * strings to netscape.
 *
 * <p>
 ...
 * </pre>
 * 
 * @see <a href="../../../etc/screenshot/gnu/app/Salt.help"> help output</a>
 */
public class Salt extends Application {
	public Salt(String[] args) {
		super(args);

		String res_name = option.string("res-name", "resource name of windows", "");
		String res_class = option.string("res-class", "resource class of windows", "");
		String message = option.string("message", "message to be sent", "");

		about("0.1", "send string to top-level windows", "Stephen Tse <stephent@sfu.ca>",
		...
		for (Window window : display.getDefaultRoot().tree().children()) {
			...
			for (int i = 0; i < message.length(); i++)
				sendKey(window, message.charAt(i));
	...
	public void sendKey(Window window, int keysym) {
		...
		if (capital)
			key_event.setState(KeyMask.SHIFT_MASK.getCode());
