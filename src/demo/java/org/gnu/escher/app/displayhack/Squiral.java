 ...
 *
 * <p>
 * Modified from <code>squiral.c</code> in
 * <a href= "http://www.jwz.org/xscreensaver/">xscreensaver</a> by Jeff Epler
...
public class Squiral extends DisplayHack {
	...
	public class Worm {
		public org.gnu.escher.x11.Color color;
		...
		public void go() {
			...
			if ((left_handed // left-handed
					&& !try_go(counter_clockwise()) && !try_go(direction) // straight
					&& !try_go(clockwise()))

					|| (!left_handed // right-handed
							&& !try_go(clockwise()) && !try_go(direction) // straight
		...
		public boolean try_go(int direction) {
			...
			if (!clear(x + dx, y + dy) || !clear(x + dx + dx, y + dy + dy))
				return false;
	...
	public Squiral(String[] args) {
		...
		disorder = option.scale("disorder", "chance a worm will choose a new direction", 0.005f);
		draw_delay = option.longg("draw-delay", "delay between iterations in millis", 10);
		fill = option.floatt("fill", "how much screen space should be filled", 0.75f, 0.01f, 0.99f);
		left_handed = option.scale("left-handed", "chance a worm goes left hand", 0.5f);
		worm_count = option.intt("worm-count", "total number of worms: 0 = window.width/32", 0);

		about("0.1", "square spirals", "Stephen Tse <stephent@sfu.ca>", "http://escher.sourceforge.net/");

		if (help_option)
			return;

		if (worm_count == 0)
			worm_count = window.width / 32;
	...
	public void paint() {
		...
		while (true) {
			for (int i = 0; i < worm_count; i++)
				worms[i].go();
			if (filled_count > fill * filled.length)
				return;
			if (sleep(draw_delay))
				return;
		}
