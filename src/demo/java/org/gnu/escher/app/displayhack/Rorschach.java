 ...
 *
 * <p>
 * Modified from <code>rorschach.c</code> in
...
public class Rorschach extends DisplayHack {
	...
	public Rorschach(String[] args) {
		...
		int step_count = option.intt("step-count", "total number of steps", 50);
		x_symmetry = option.booleann("x-symmetry", "if symmetry along x-axis", true);
		y_symmetry = option.booleann("y-symmetry", "if symmetry along y-axis", false);

		about("0.1", "simulate ink-blot patterns", "Stephen Tse <stephent@sfu.ca>", "http://escher.sourceforge.net/");
	...
	public void paint() {
		...
		for (int i = 0; i < depth; i++) {
			...
			if (x_symmetry)
				window.point(gc, xlim - x, y);
			if (y_symmetry)
				window.point(gc, x, ylim - y);
			if (x_symmetry && y_symmetry)
				window.point(gc, xlim - x, ylim - y);
