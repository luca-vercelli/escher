...
/**
 * Draw tacky 70s basement wall panelling. It subdivides and colors rectangles
 * randomly. It looks kind of like Brady-Bunch-era rec-room wall paneling.
 * (Raven says: "this screensaver is ugly enough to peel paint.")
 * 
 * <p>
 * Modified from <code>deco.c</code> in
 * <a href= "http://www.jwz.org/xscreensaver/">xscreensaver</a> by Jamie
 * Zawinski <jwz@jwz.org>.
 *
 * @see <a href="../../../../etc/screenshot/gnu/app/displayhack/Deco.gif">
 *      screenshot 8</a>
 * 
 * @see <a href="../../../../etc/screenshot/gnu/app/displayhack/Deco.help"> help
 *      output</a>
 */
public class Deco extends DisplayHack {
	...
	public Deco(String[] args) {
		...
		max_depth = option.intt("max-depth", "maximum level of recursion", 12);
		min_size = option.intt("min-size", "mimimum size of width and height", 20);

		about("0.1", "draw tacky 70s basement wall panelling", "Stephen Tse <stephent@sfu.ca>",
	...
	public void deco(int x, int y, int width, int height, int depth) {
		if (width < min_size || height < min_size || random.nextInt(max_depth) > depth) { // stop recursion
			 ...
			 *
			 * Note rounding error in width/2 + widht/2 != width!!! Use
			 * <code>width-width/2</code> to get ceiling.
			 */
