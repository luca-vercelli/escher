 ...
 *
 * <p>
 * Modified from <code>sprite.c</code> in
 * <a href="http://xfree86.org/~keithp/download/test.tar.bz2">render sample
...
public class Sprites extends DisplayHack {
	...
	class Sprite {
		...
		public Sprite(float dx, float dy, int color, int width, int height, PictFormat pf) {
			...
			pixmap = new Pixmap(display.getDefaultRoot(), width, height, pf.depth());
			picture = render.create_picture(pixmap, pf, DrawablePicture.Attributes.EMPTY);
		...
		public void clear() {
			back_buffer.rectangle(back_buffer_gc, (int) x, (int) y, width, height, true);
		}
		...
		public void move() {
			...
			render.composite(Render.OVER, picture, Picture.NONE, back_buffer_picture, 0, 0, 0, 0, (int) x, (int) y,
					width, height);
		}
	}
	...
	public Sprites(String[] args) throws org.gnu.escher.x11.extension.ExtensionNotFoundException {
		...
		back_buffer_picture = render.create_picture(back_buffer, pf1, DrawablePicture.Attributes.EMPTY);
	...
	public void paint() {
		...
		for (int i = 0; i < COUNT; i++)
			sprites[i].move();
		window.copyArea(back_buffer, back_buffer_gc, 0, 0, back_buffer.width, back_buffer.height, 0, 0);
	}
