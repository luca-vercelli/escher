 ...
 * 
 * You can find out WM-NAME and WM-CLASS of any window using standard X Window
 * System utilities such as `xwininfo' and `xprop', or you can use
...
public class Preference {
	 ...
	 * My desktop.
	 *
	 * licq 0.85 pclock -w -B $HOME/lib/ugly-yellow.xpm -H blue wmload -exe puppet
	 * xeyes xsetbg $HOME/lib/echer.jpg
	 */
	...
	private static final Point[] WARP = { new Point(60, 1015), // 0 - licq message
			null, // 1
			null, // 2
			null, // 3
			null, // 4
	...
	private static final Map<String, Object> hash = new HashMap<>(29);
	static {
		// no focus
		for (int i = 0; i < NO_FOCUS.length; i++) {
			String key = NO_FOCUS[i][0] + "\0" + NO_FOCUS[i][1] + ":" + "no-focus";
			hash.put(key, Boolean.TRUE);
		}

		// no geometry change
		for (int i = 0; i < NO_GEOMETRY_CHANGE.length; i++) {
			String key = NO_GEOMETRY_CHANGE[i][0] + "\0" + NO_GEOMETRY_CHANGE[i][1] + ":" + "no-geometry-change";
			hash.put(key, Boolean.TRUE);
		}

		// position
		for (int i = 0; i < POSITION.length; i++) {
			String key = POSITION[i][0] + "\0" + POSITION[i][1] + ":" + "position";
		...
		for (int i = 0; i < REGISTER.length; i++) {
			...
			String key = REGISTER[i][0] + "\0" + REGISTER[i][1] + ":" + "register";
	...
	public String launch(int i) {
		return LAUNCH[i];
	}

	public String launch_on_root() {
		return LAUNCH_ON_ROOT;
	}
