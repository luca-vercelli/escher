 ...
 *
 * <p>
 * Modified from <code>blend.c</code> in
 ...
 *      screenshot</a>
 * 
 * @see <a href="../../../../etc/screenshot/gnu/x11/test/Blend.help"> help
 *      output</a>
 */
public class Blend extends Graphics {
	...
	public Blend(String[] args) throws org.gnu.escher.x11.extension.ExtensionNotFoundException {
		...
		window_picture = render.create_picture(window, pf1, DrawablePicture.Attributes.EMPTY);
		...
		alpha_picture = render.create_picture(alpha_pixmap, pf1, DrawablePicture.Attributes.EMPTY);
	...
	public void paint() {
		...
		alpha_pixmap.rectangle(alpha_gc, 0, 0, window.width, window.height, true);
		alpha_gc.setForeground(0x7f);
		alpha_pixmap.rectangle(alpha_gc, 10, 10, 50, 50, true);

		render.composite(Render.OVER, color_picture, alpha_picture, window_picture, 0, 0, 0, 0, 0, 0, window.width,
				window.height);
