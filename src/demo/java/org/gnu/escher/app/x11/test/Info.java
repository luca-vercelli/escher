 ...
 *
 * @see <a href="../../../../etc/screenshot/gnu/x11/test/Info.output"> text
 *      output on linux + xfree86 4.0</a>
 *
 ...
 * 
 * @see <a href="../../../../etc/screenshot/gnu/x11/test/Chinese.help"> help
 *      output</a>
 */
public class Info extends Application {
	public Info(String[] args) {
		...
		boolean print_keysyms = option.booleann("print-keysyms", "print all keysyms for debug", false);

		about("0.1", "list info about X server", "Stephen Tse <stephent@sfu.ca>", "http://escher.sourceforge.net/");
		...
		System.out.println("\n\n---- screens" + Arrays.toString(display.getScreens()));

		System.out.println("\n\n---- pixmap formats" + Arrays.toString(display.getPixmapFormats()));
		...
		System.out.println(
				"  keycode-count: " + (display.getInput().getMaxKeycode() - display.getInput().getMinKeycode() + 1));
		System.out.println("  keysyms-per-keycode: : " + display.getInput().getKeysymsPerKeycode());

		// compare to "xmodmap -pk"
		if (print_keysyms) {
			System.out.println("  ** keysyms **");
			for (int i = 0; i < display.getInput().getKeySymsLength(); i++)
				System.out.println(i + display.getInput().getMinKeycode() + ": " + display.getInput().getKeysym(i));
		}
	}
	...
	public void extension_details() {
		...
		try {
			System.out.println(new org.gnu.escher.x11.extension.BigRequests(display) + "\n");
		...
		try {
			...
			GL gl = glx.create_context(display.getDefaultScreen().getRootVisualID(), display.getDefaultScreenNumber(),
					GL.NONE0);
		...
		try {
			...
			System.out.println(render + Arrays.toString(render.picture_formats()));
		} catch (ExtensionNotFoundException e) {
