 ...
 * 
 * <p>
 * The implementation uses two threads so that one can be painting and one can
 * be waiting for user input. Only with two threads can {@link #sleep(long)} be
 * interrupted (with {@link #restart()} or {@link #exit()}) to give responsive
 * user interface.
 */
public abstract class DisplayHack extends Application implements Runnable {
	...
	/**
	 * Clear window before the first iteration. Hacks that play with screen content
	 * or those that wish to improve initial perceived drawing speed (such as
	 * {@link Deco}) should set this to false.
	 */
	...
	public DisplayHack(String[] args, boolean clear, boolean erase, boolean rainbow_color, int default_color_count,
			int default_delay) {
		...
		int color_count = option.intt("color-count", "total number of random colors", default_color_count);
		delay = option.longg("delay", "delay between screens in ms", default_delay);

		if (erase) {
			eraser_delay = option.longg("eraser-delay", "delay between iterations of eraser in ms", 10);
			eraser_delta = option.intt("eraser-delta", "granularity of eraser", 5, 1, 10);
			eraser_mode = option.enumerate("eraser-mode", "which eraser", Eraser.ALL_STRINGS,
					Eraser.RANDOM_ERASER_INDEX);
		}

		Rectangle geometry = option.rectangle("geometry", "initial geometry of main window",
		...
		win_attr.setEventMask(EventMask.BUTTON_PRESS_MASK.getMask() | EventMask.STRUCTURE_NOTIFY_MASK.getMask()
	...
	public void about(String version, String description, String author, String url) {
	...
	public void dispatch_event() {
		...
		case KEY_PRESS: {
			...
			if (keysym == ' ')
				restart();
			else if (keysym == 'q' || keysym == 'Q' || keysym == gnu.x11.keysym.Misc.ESCAPE)
				exit();
			break;
	 ...
	 * constructors to be called before dispatching the first <code>Expose</code>.
	...
	public void exec() {
		...
		/*
		 * Do not close display here. The other thread (painting thread) is reponsible
		 * for closing display, because it always outlasts the main thread.
		 */
	}
